...
describe("Menu Service Integration Tests (database)", () => {
  ...
  describe("findById", () => {
    it("should return a single item from the database", async () => {
      const item = await getAnyMenuItem();

      const result = (await service.findById(item.id).execute()) as FindOneResult;
    ...
    it("should throw NotFoundError if the item does not exist", async () => {
  ...
  describe("findMany", () => {
    it("should query the database with the default state", async () => {
  ...
  describe("Query Building and Chaining", () => {
    it("should build state for findByCategoryId", async () => {
    ...
    it("should build state for findByPriceRange", async () => {
      ...
      const values = prices.map((p) => p.price as number);
    ...
    it("should build state for search", async () => {
      const [item] = await db.select().from(menuItems).limit(1);
      if (!item) throw new Error("Test database must contain at least one menu item");
    ...
    it("should build state for page", async () => {
      const pagination = { page: 2, perPage: 1 };
      const page1 = (await service
        .page({ page: 1, perPage: 1 })
        .execute()) as FindManyResult;
      const page2 = (await service.page(pagination).execute()) as FindManyResult;
    ...
    it("should chain multiple state modifiers correctly", async () => {
      const { category } = await getAnyCategoryWithItems();
    ...
    it("should allow searching and then filtering", async () => {
      const [item] = await db.select().from(menuItems).limit(1);
      if (!item) throw new Error("Test database must contain at least one menu item");
    ...
    it("should override id when chaining", async () => {
      ...
      const manyAfter = (await service.findMany().execute()) as FindManyResult;
